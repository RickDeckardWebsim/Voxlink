...
`message_id` is generated by the UI thread (`Uuid::new_v4()` in `try_send_message`, Task 6) *before* the command is sent, so the same UUID is used for the broadcast `message_id`, the optimistic message's `db_id`, and the DB insert `id`. The webrtc dispatch (Task 5) only forwards it — it does not generate it.
...
**Step 2: Update the dispatch — forward `message_id` + reply**

The `message_id` is generated in the UI thread (`try_send_message`, Task 6) and arrives on the command; this dispatch only forwards it to the signaling task. It does **not** generate a UUID.
...
**Files:**
- Modify: `web/app.js` (`trySend` `:811-824`, `uploadMedia` `:826-854`, `onChatMessage` `:480-483`, `onChatMedia` `:485-488`, `fetchHistory` `:777-808`, `appendMsg` `:859-922`, `onReaction` already uses `dataset.msgId`)
...
**Step 4: `fetchHistory` — select + pass reply fields from history**

`fetchHistory` (`web/app.js:777-808`) currently selects no `reply_to_*` columns and passes no reply data to `appendMsg` (line 794), so replies loaded from history would silently lose their quoted reference. Extend the `.select()` string at `app.js:780` and pass reply data into the `appendMsg` call at line 794:
...
This mirrors the live `onChatMessage`/`onChatMedia` path (Step 3) so history-loaded replies render identically to live ones.

**Step 5: `appendMsg` — accept `dbId` (already does) + `reply`, render `.msg-reply-ref`**
...
**Step 6: Reply affordance — hover "Reply" button + preview bar**
...
**Step 7: Mention highlight in content**
